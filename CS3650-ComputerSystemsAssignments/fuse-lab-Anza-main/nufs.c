...
int nufs_access(const char *path, int mask) {
  ...
  if (strcmp(path, "/") == 0 || strcmp(path, "/myworld.txt") == 0 || strcmp(path, "/test.txt") == 0) {
    rv = 0;
...
int nufs_getattr(const char *path, struct stat *st) {
  ...
  // ...and the simulated file...
    ...
    st->st_mode = 0100644; // regular file
    st->st_size = 11; // Size of "0123456789"
    st->st_uid = getuid();
  }
...
int nufs_readdir(const char *path, void *buf, fuse_fill_dir_t filler,
  ...
  if (strcmp(path, "/") != 0) {
    return -ENOENT; // Only handle the root directory
  }
...
int nufs_read(const char *path, char *buf, size_t size, off_t offset,
 ...
 // strcpy(buf, "hello\n");
  ...
  if (strcmp(path, "/myworld.txt") == 0) {
        // Simulate the content of /hello.txt
        const char *content = "myworld\n";
        size_t content_size = strlen(content);

        // Handle the offset and size for partial reads
        if (offset < content_size) {
            size_t bytes_to_copy = (size < (content_size - offset)) ? size : (content_size - offset);
            memcpy(buf, content + offset, bytes_to_copy);
            rv = bytes_to_copy;
        }
  }
  else if (strcmp(path, "/test.txt") == 0) {
        // Simulate the content of /test.txt
